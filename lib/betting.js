...
module.exports = function (session) {
  ...
  return {
     ...
     * @param cb
     */
    placeOrders: function (marketId, instructions, async, customerRef, marketVersion, customerStrategyRef, cb) {
      if (!marketId) {
      ...
      if(marketVersion) {
        req.data.marketVersion = { version: parseFloat(marketVersion)};
      }

      if(customerStrategyRef) {
        if(customerStrategyRef.length > 15) {
          customerStrategyRef = customerStrategyRef.substring(0, 14);
        }
        req.data.customerStrategyRef = customerStrategyRef;
      }
